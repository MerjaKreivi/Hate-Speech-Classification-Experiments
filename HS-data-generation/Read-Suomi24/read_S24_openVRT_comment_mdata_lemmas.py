# read_S24_openVRT_comment_mdata_lemmas.py
""" 
...
Corresponds to the defined data structure of range(11)
...
def extract_meta(line):
    # below an example of new (2020) metadata style
...
def read_S24(S24file):
    ...
    for line in S24file:
        ...
        if line.startswith("<text comment"):
            skipTextComment = False
            if len(text) > 0:
                yield attribute, meta, text, lemmas
            attribute = ""
            meta = []
            text = ""
            lemmas = ""
...
def readNewFormat(vrtFile):    
    ...
    with open(vrtFile, encoding='utf8') as infile:
        ...
        for attribute, meta, text, lemmas in read_S24(infile):
